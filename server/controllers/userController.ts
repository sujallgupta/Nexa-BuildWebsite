...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user && user.credits < 5) {
            return res
                .status(403)
                .json({ message: "Add credits to build more projects" });
        }
...
`,
        ...
        await prisma.conversation.create({
            data: {
                role: "assistant",
                content:
                    "Your website is live and ready to preview! Feel free to explore it and request any changes or enhancements.",
                projectId: project.id,
            },
        });
...
export const PurchaseCredits = async (
    ...
    try {
        ...
        const plans = {
            starter: { credits: 40, amount: 5 },
            pro: { credits: 200, amount: 28 },
            enterprise: { credits: 1000, amount: 90 },
            ...
            line_items: [
                {
                    price_data: {
                        currency: 'usd',
                        product_data: {
                            name: `Nexa - Build Apps & Website - ${plan.credits} credits `
                        },
